...
class OpenBASMicrosoftAzure:
    def __init__(self):
        ...
        self.resource_groups = self.config.get_conf("microsoft_azure_resource_groups", default="")
        
        # Parse resource groups (comma-separated)
        self.resource_groups_list = [rg.strip() for rg in self.resource_groups.split(',') if rg.strip()]
    ...
    def _get_access_token(self):
        ...
        try:
            ...
            result = app.acquire_token_silent([f"{self.resource}/.default"], account=None)
            if not result:
                result = app.acquire_token_for_client(scopes=[f"{self.resource}/.default"])
            
            if "access_token" in result:
                self.access_token = result["access_token"]
                self.helper.collector_logger.info("Successfully authenticated with Azure")
                return True
            else:
    ...
    def _azure_api_call(self, endpoint):
        ...
        # Add API version for VM endpoints
        if "?" not in url:
                ...
                if nic_id:
                    ...
                    if len(parts) >= 9:
                        ...
                        if nic_data and "properties" in nic_data:
                            for ip_config in nic_data["properties"].get("ipConfigurations", []):
                                if "properties" in ip_config:
                                    ...
                                    if "publicIPAddress" in ip_config["properties"]:
                                        public_ip_id = ip_config["properties"]["publicIPAddress"].get("id", "")
                                        if public_ip_id:
                                            ...
                                            if len(pub_parts) >= 9:
                                                ...
                                                pub_data = self._azure_api_call(pub_endpoint)
                                                if pub_data and "properties" in pub_data:
                                                    public_ip = pub_data["properties"].get("ipAddress")
                                                    if public_ip:
    ...
    def _process_message(self) -> None:
        ...
        try:
            ...
            if not self.resource_groups_list:
                self.helper.collector_logger.info("No specific resource groups provided, collecting all VMs in subscription")
                ...
                for resource_group in self.resource_groups_list:
                    self.helper.collector_logger.info(f"Collecting VMs from resource group: {resource_group}")
                    vms = self._get_vms_from_resource_group(resource_group)
                    all_vms.extend(vms)
                    self.helper.collector_logger.info(f"Found {len(vms)} VMs in resource group {resource_group}")
            ...
            for vm in all_vms:
                ...
                if provisioning_state.lower() != "succeeded":
                    self.helper.collector_logger.warning(f"Skipping VM {vm_name} - provisioning state: {provisioning_state}")
                    continue
                ...
                endpoint = {
                    ...
                    "endpoint_arch": "x86_64" if "64" in vm_size or platform != "Generic" else "Unknown",
                    "endpoint_ips": ips if ips else [],
                ...
                try:
                    ...
                    self.helper.collector_logger.info(f"Successfully upserted endpoint: {vm_name}")
                except Exception as e:
                    self.helper.collector_logger.error(f"Failed to upsert endpoint {vm_name}: {str(e)}")

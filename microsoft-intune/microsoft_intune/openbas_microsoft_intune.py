...
class OpenBASMicrosoftIntune:
    def __init__(self):
        ...
        self.device_filter = self.config.get_conf("microsoft_intune_device_filter", default="")
        self.device_groups = self.config.get_conf("microsoft_intune_device_groups", default="")
        
        # Parse device groups (comma-separated)
        self.device_groups_list = [g.strip() for g in self.device_groups.split(',') if g.strip()]
    ...
    def _get_access_token(self):
        ...
        try:
            ...
            if "access_token" in result:
                ...
                self.helper.collector_logger.info("Successfully authenticated with Microsoft Graph")
                return True
            else:
                self.helper.collector_logger.error(f"Failed to get access token: {result.get('error_description', 'Unknown error')}")
                return False
    ...
    def _graph_api_call(self, endpoint, params=None):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.helper.collector_logger.error(f"API call failed: {response.status_code} - {response.text}")
                return None
        except Exception as e:
    ...
    def _get_managed_devices(self):
        ...
        if self.device_groups_list:
            self.helper.collector_logger.info(f"Filtering by device groups: {', '.join(self.device_groups_list)}")
            ...
            for group_filter in self.device_groups_list:
                ...
                for group in all_groups:
                    # Match by name or ID
                    if group_filter.lower() in [group.get("displayName", "").lower(), group.get("id", "").lower()]:
                        group_found = True
                        self.helper.collector_logger.info(f"Processing group: {group.get('displayName')} ({group.get('id')})")
                        
                        # Get devices from this group
                        device_ids = self._get_devices_from_group(group.get("id"))
                        allowed_device_ids.update(device_ids)
                        self.helper.collector_logger.info(f"Found {len(device_ids)} devices in group {group.get('displayName')}")
                
                if not group_found:
                    self.helper.collector_logger.warning(f"Device group '{group_filter}' not found")
            
            if not allowed_device_ids:
                self.helper.collector_logger.warning("No devices found in specified groups")
                return []
    ...
    def _determine_architecture(self, device):
        ...
        elif any(mobile_os in os_type for mobile_os in ["android", "ios", "ipad", "iphone"]):
            return "arm64"
        # Apple Silicon Macs
        elif "mac" in os_type and "apple" in manufacturer:
            # Newer Macs might be ARM
            return "arm64" if any(year in model for year in ["2020", "2021", "2022", "2023", "2024", "2025"]) else "x86_64"
    ...
    def _create_or_get_tag(self, tag_name, tag_color="#6b7280"):
        ...
        try:
            ...
            self.helper.collector_logger.warning(f"Failed to upsert tag {tag_name}: {e}")
            return None
    ...
    def _process_message(self) -> None:
        ...
        try:
            self.helper.collector_logger.info("Starting Microsoft Intune device collection...")
            
            # Get all managed devices
            devices = self._get_managed_devices()
            self.helper.collector_logger.info(f"Found {len(devices)} managed devices in Intune")
            ...
            for device in devices:
                ...
                if device.get("deviceEnrollmentType"):
                    tag_name = f"enrollment:{device.get('deviceEnrollmentType').lower()}"
                ...
                if device.get("deviceCategoryDisplayName"):
                    tag_name = f"category:{device.get('deviceCategoryDisplayName').lower()}"
                ...
                if manufacturer and manufacturer.lower() not in ["unknown", "n/a", ""]:
                    ...
                    tag_id = self._create_or_get_tag(tag_name, tag_colors["manufacturer"])
                    if tag_id:
                        tag_ids.append(tag_id)
                ...
                if model and model.lower() not in ["unknown", "n/a", ""]:
                    ...
                    model_clean = model[:50].replace("/", "-").replace("\\", "-").replace(" ", "-")
                ...
                # Add OS tag
                if os_type:
                    os_tag = os_type.split()[0].lower() if " " in os_type else os_type.lower()
                ...
                if device.get("isEncrypted"):
                    tag_id = self._create_or_get_tag("security:encrypted", tag_colors["encrypted"])
                    if tag_id:
                        tag_ids.append(tag_id)
                
                if device.get("isSupervised"):
                    tag_id = self._create_or_get_tag("management:supervised", tag_colors["supervised"])
                    if tag_id:
                        tag_ids.append(tag_id)
                ...
                try:
                    ...
                    self.helper.collector_logger.info(f"Successfully upserted endpoint: {device_name}")
                except Exception as e:
                    self.helper.collector_logger.error(f"Failed to upsert endpoint {device_name}: {str(e)}")
            
            self.helper.collector_logger.info("Microsoft Intune device collection completed")
            
        except Exception as e:
            self.helper.collector_logger.error(f"Error during device collection: {str(e)}")

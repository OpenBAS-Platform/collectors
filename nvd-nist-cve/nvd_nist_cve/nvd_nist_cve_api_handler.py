...
class NvdNistCveApiHandler:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if error_data.get("message") == "Invalid apiKey.":
                    raise Exception("[API] Invalid API Key provided. Please check your configuration.")
                else:
    ...
    def __fetch_cve_data_by_page(self, cve_params: dict, start_index=0) -> Generator[CVEFetchResult]:
        """
        ...
        while is_finished is False:
            ...
            if not vulnerabilities:
                ...
                cve_params_with_rate["startIndex"] += cve_params_with_rate["resultsPerPage"]
    ...
    def _get_vulnerabilities_by_date_range(self, start_date: datetime, end_date: datetime) -> Generator[CVEFetchResult]:
        """
        ...
        while current_start_date < end_date:
            ...
            cve_params = self._update_cve_params_date(current_start_date, current_end_date)
            self.logger.info(f"Batching from {current_start_date} to {current_end_date}")
    ...
    def _get_vulnerabilities_by_index(self, start_index=0) -> Generator[CVEFetchResult]:
        ...
        for result in self.__fetch_cve_data_by_page({}, start_index):
            result.last_mod_date= datetime.fromisoformat(result.vulnerabilities[len(result.vulnerabilities) - 1].get("cve", {}).get("lastModified"))
            yield result
